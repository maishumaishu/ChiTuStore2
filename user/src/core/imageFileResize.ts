...
class ImageFileResize {
    ...
    constructor(fileUploadElement: HTMLInputElement, thumb1: ImageThumb, thumb2: ImageThumb = null) {
        ...
        fileUploadElement.onchange = () => {
            ...
            for (var i = 0; i < fileUploadElement.files.length; i++) {
                this.processfile(fileUploadElement.files[i], i, maxWidth, maxHeight).then((data: FileData) => {
    ...
    private processfile(file: File, index, max_width: number, max_height: number): Promise<FileData> {
        ...
        return new Promise<FileData>((resolve, reject) => {
            ...
            reader.onload = (ev: Event) => {
                ...
                image.onload = () => {
                    ...
                    resolve({ index: index, url: url, data: url, thumb: thumb });
                }
            }
        })
