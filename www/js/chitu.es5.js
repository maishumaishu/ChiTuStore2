                ...
                for (var _i = 0; _i < items.length; _i++) {
                    for (var _j = _i + 1; _j < items.length; _j++) {
                        if (items[_i].name == items[_j].name) {
                            throw chitu.Errors.resourceExists(items[_i].name, routeData.pageName);
                        }
                    }
                }
                return this._push.apply(this, items);
            };
        }
        ...
        _createClass(Application, [{
            ...
            value: function showPage(routeString, args) {
                ...
                var result = new Promise(function (resolve, reject) {
                    var page = _this2.createPage(routeData);
                    _this2.on_pageCreated(page);
                    page.show();
                    resolve(page);
                });
                return result;
